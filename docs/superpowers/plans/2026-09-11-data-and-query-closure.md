...
- **受控 SQL 探索：** 状态：计划完成；实现以 Task 11 门禁为前置（Task 1–6 的本地实现与本地验收已完成，生产启用与真实模型未执行）。[实施计划](2026-09-14-controlled-sql-exploration.md)、[本地验收记录](../research/2026-09-14-controlled-sql-acceptance.md)。在语义目录之后交付，只承接固定 Tool 无法表达且能力/覆盖完整的问题，使用服务端 SQL 编译、AST、只读权限与预算纵深防御；交付面叫**受控聚合探索**，不是通用 Text2SQL，也没有任一个“用户递 SQL”的入口，`CONTROLLED_SQL_ENABLED` 默认关闭。
