...
- [ ] **5.0c 核验历史修复（未完成：before 已取证，回放未执行）。** 先在独立测试库执行既有订单 replay，重建支付与匹配，核对每店原始金额/orphan/未匹配/补拉集合的前后变化及批次；通过后再按运行手册发布到真实库。增量同版本重跑不能替代修复。
  2026-09-12 生产库只读取证已固化：关闭已付款单 1864、未认证支付 300 笔（金额全 NULL）、全库未匹配 canonical 成功退款 42、166754 补拉集合 27 个 cid；014/015 未应用，7 家店的 capabilities 仍是实体标签、quality_rule 仍是 `kuaimai-reconcile/1`。见 [前置取证记录](../research/2026-09-12-production-preflight-5-0c.md)。下一步按该记录的四步顺序执行。
